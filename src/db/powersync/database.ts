...
/**
 * Prebuilt UMD worker files served from public/@powersync/worker/.
 * Using explicit paths avoids import.meta.url resolution failures in Tauri production builds.
 * Files are generated by: bunx @powersync/web copy-assets --output public
 */
...
export class PowerSyncDatabaseImpl implements DatabaseInterface {
  ...
  async initialize(path: string): Promise<void> {
    ...
    //
    // On iOS, OPFSCoopSyncVFS is required: it uses synchronous OPFS access handles with no
    // Asyncify and no SharedArrayBuffer, avoiding both the WKWebView WebContent process OOM
    // (~30s black screen) and the JavaScriptCore Asyncify stack overflow caused by the default
    // IDBBatchAtomicVFS. See: https://docs.powersync.com/debugging/troubleshooting
    //
    // On all platforms, explicit UMD worker paths are used to guarantee Tauri production builds
    // can locate the worker scripts (import.meta.url resolution is unreliable in Tauri).
    // Worker files are pre-copied to public/@powersync/worker/ via copy-assets.
    // Tauri is always single-window, so multi-tab sync is never needed.
    // Keeping enableMultiTabs: false avoids SharedWorker usage, which fails in Tauri
    // production builds due to the tauri:// custom protocol.
    ...
    const options: WebPowerSyncDatabaseOptions = {
      ...
      }) as unknown as WebPowerSyncDatabaseOptions['database'],
